...
export function InterviewResult({ id }: { id: string }) {
  ...
  if (!data) return <div className="flex items-center justify-center py-24"><Loader2 className="size-7 animate-spin text-slate-500" /></div>;

  if (data.status !== 'completed') {
    return (
      <div className="py-16 text-center">
        <p className="text-sm text-slate-600">This interview isn&apos;t finished yet.</p>
  ...
  if (!evalv) {
    return (
      <div className="flex flex-col items-center gap-3 py-20">
        ...
        <p className="text-sm text-slate-600">Evaluating your interview…</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <Link href="/mock-interview" className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 hover:text-navy">
      ...
      >
        ...
        <div className="relative grid size-32 shrink-0 place-items-center">
          ...
          <div className="absolute flex flex-col items-center">
            ...
            <span className="-mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">out of 100</span>
          </div>
        </div>
        <div className="relative">
          <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">{data.topic} · {data.difficulty} · {data.interviewType}</p>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-green-200 bg-green-50/50 p-4">
          ...
          {evalv.strengths.length ? <ul className="list-disc space-y-1 pl-4 text-sm text-slate-700">{evalv.strengths.map((s, i) => <li key={i}>{s}</li>)}</ul> : <p className="text-sm text-slate-500">-</p>}
        </div>
        <div className="rounded-xl border border-amber-200 bg-amber-50/50 p-4">
          ...
          {evalv.areas_for_improvement.length ? <ul className="list-disc space-y-1 pl-4 text-sm text-slate-700">{evalv.areas_for_improvement.map((s, i) => <li key={i}>{s}</li>)}</ul> : <p className="text-sm text-slate-500">-</p>}
        </div>
      </div>

      {/* Per-question */}
      <section className="space-y-3">
        <h3 className="text-sm font-bold uppercase tracking-widest text-slate-500">Question breakdown</h3>
