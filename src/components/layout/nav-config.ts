...
export const SUPERADMIN_NAV: NavSection[] = [
  ...
  {
    // The job board lives in the /admin console, and the API has always allowed a
    // SUPER_ADMIN there (@Roles(ADMIN, SUPER_ADMIN)). Without these links the six
    // super-admin accounts - which is what the owner signs in as - had no way to
    // reach it but to type the URL.
    heading: 'JOBS',
    items: [
