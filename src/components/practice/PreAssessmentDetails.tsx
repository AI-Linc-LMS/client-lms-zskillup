...
/**
 * One-time pre-assessment details gate. Before a scheduled assessment starts we make
 * sure the report will carry Student name, College, Email and Contact number. Only the
 * fields the student is MISSING are asked; anything already on their profile (set by
 * them or by their college) is shown locked and never re-asked. Saved to the profile
 * via PATCH /me, so it is a genuinely one-time step — the next assessment sees the
 * fields present and this never appears. The caller only mounts this when at least one
 * field is missing, so a fully-filled student never sees it at all.
 */

/** True when the profile is missing any of the four report fields. */
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <h2 className="text-lg font-bold text-navy">Confirm your details</h2>
