  ...
  return (
    <div className="px-10 py-9 text-slate-800">
      <header className="mb-4">
        ...
        <p className="mt-1 font-mono text-[11px] text-slate-600">{socialList(data).map((s) => `${s.value}`).join('  |  ')}</p>
      </header>
      ...
      {data.workExperience.length > 0 && (
        <section className="mb-4">{head('experience')}
          {data.workExperience.map((w) => (
            <div key={w.id} className="mb-2.5"><div className="flex items-baseline justify-between"><p className="text-[13px] font-bold">{w.position} · {w.company}</p><span className="font-mono text-[11px] text-slate-600">{dateRange(w.startDate, w.endDate, w.current)}</span></div><Bullets items={w.description} /></div>
          ))}
        </section>
      )}
  ...
  return (
    <div className="flex min-h-full">
      ...
      <div className="flex-1 px-10 py-9 text-slate-800">
        ...
        {b.summary && <section className="mb-4">{head('Summary')}<p className="text-[12.5px] leading-snug text-slate-700">{b.summary}</p></section>}
        {data.workExperience.length > 0 && (
          <section className="mb-4">{head('Experience')}{data.workExperience.map((w) => (<div key={w.id} className="mb-2.5"><div className="flex items-baseline justify-between"><p className="text-[13px] font-bold">{w.position}</p><span className="text-[11px] text-slate-600">{dateRange(w.startDate, w.endDate, w.current)}</span></div><p className="text-[12px] font-medium text-rose-600">{w.company}</p><Bullets items={w.description} /></div>))}</section>
        )}
  ...
  return (
    <div className="flex min-h-full text-slate-800">
      ...
      <aside className="w-[32%] bg-slate-100 px-6 py-8">
        ...
        <div className="space-y-1.5 text-[11.5px] text-slate-700">{socialList(data).map((s) => (<p key={s.label} className="break-words"><span className="font-semibold text-slate-600">{s.label}: </span>{s.value}</p>))}</div>
  ...
  return (
    <div className="px-9 py-8 text-slate-800">
      <header className="mb-5 flex items-center gap-5">
        ...
        <div><h1 className="text-[30px] font-black leading-none text-slate-900">{fullName(b) || 'Your Name'}</h1><p className="mt-1 text-sm font-bold uppercase tracking-[0.2em] text-amber-700">{b.professionalTitle}</p><p className="mt-1.5 text-[11px] text-slate-600">{socialList(data).map((s) => s.value).join('  •  ')}</p></div>
      </header>
  ...
  return (
    <div className="px-14 py-12 font-serif text-slate-800">
      <header className="mb-6 text-center">
        ...
        <p className="mt-2 text-[11px] text-slate-600">{socialList(data).map((s) => s.value).join('   ·   ')}</p>
      </header>
  ...
  return (
    <div className="bg-sky-50/40 px-9 py-8 text-slate-800">
      <header className="mb-5 flex items-center gap-4 rounded-2xl bg-white p-4 shadow-sm">
        ...
        <div><h1 className="text-[26px] font-black text-slate-900">{fullName(b) || 'Your Name'}</h1><p className="text-sm font-semibold text-sky-600">{b.professionalTitle}</p><p className="mt-0.5 text-[11px] text-slate-600">{socialList(data).map((s) => s.value).join('  •  ')}</p></div>
      </header>
      ...
      {data.workExperience.length > 0 && <section className="mb-4 rounded-2xl bg-white p-4 shadow-sm">{head('Experience')}{data.workExperience.map((w) => (<div key={w.id} className="mb-2.5"><div className="flex items-baseline justify-between"><p className="text-[13px] font-bold">{w.position}</p><span className="text-[11px] text-slate-600">{dateRange(w.startDate, w.endDate, w.current)}</span></div><p className="text-[12px] font-medium text-sky-600">{w.company}</p><Bullets items={w.description} /></div>))}</section>}
